				...
				if (response.is_role_applicable && response.can_submit_to_workflow) {
					...
					if (!this.state.actionHistoryId && !this.props.postMeta._oasis_is_in_workflow && response.can_submit_to_workflow) {
						...
						workflowButtonDiv.innerHTML = '<button type="button" aria-label="'
						 ...
						 /**
						 * Check first class "edit-post-header__settings" exists if not then check class "editor-header__settings"
						 * If not exists then stop continute any further.
						 * From WordPress v6.6 "edit-post-header__settings" are no longer exists. it's now "editor-header__settings"
						 * 
						 * @since 9.9
						 */
